#!/usr/bin/env bash
# =============================================================================
# Rider App Build and Deploy Script
# =============================================================================
...
GIT_USER=${GIT_USER:-"Rider App Bot"}
...
NEXT_PUBLIC_LOGISTICS_API_URL=${NEXT_PUBLIC_LOGISTICS_API_URL:-"https://logisticsapi.codevertexafrica.com/api/v1"}
NEXT_PUBLIC_SSO_URL=${NEXT_PUBLIC_SSO_URL:-"https://sso.codevertexafrica.com"}
NEXT_PUBLIC_APP_URL=${NEXT_PUBLIC_APP_URL:-"https://riderapp.codevertexafrica.com"}
...
if [[ ${DEPLOY} == "true" ]]; then
  ...
  if curl -fsSL "https://raw.githubusercontent.com/${DEVOPS_REPO}/main/scripts/tools/check-and-sync-secrets.sh" -o "$SYNC_SCRIPT" 2>/dev/null; then
    source "$SYNC_SCRIPT"
    check_and_sync_secrets "REGISTRY_USERNAME" "REGISTRY_PASSWORD" "KUBE_CONFIG" "NEXT_PUBLIC_LOGISTICS_API_URL" || warn "Secret sync failed"
    rm -f "$SYNC_SCRIPT"
  else
...
if [[ ${DEPLOY} == "true" ]]; then
  ...
  if [[ -n ${REGISTRY_USERNAME:-} && -n ${REGISTRY_PASSWORD:-} ]]; then
    kubectl -n "$NAMESPACE" create secret docker-registry registry-credentials \
      --docker-server="${REGISTRY_SERVER:-docker.io}" \
      --docker-username="$REGISTRY_USERNAME" \
      --docker-password="$REGISTRY_PASSWORD" \
      --dry-run=client -o yaml | kubectl apply -f - || warn "registry-credentials creation failed"
  fi
  # Ensure rider-app-secrets exists (values expect mapboxToken, optional)
  SECRET_NAME="${APP_NAME}-secrets"
  if ! kubectl get secret "$SECRET_NAME" -n "$NAMESPACE" >/dev/null 2>&1; then
    info "Creating secret $SECRET_NAME in namespace $NAMESPACE (mapboxToken)..."
